...
test("gemini adapter builds official native endpoints and uses x-goog-api-key", () => {
  ...
  assert.ok(attempts.every((attempt) => JSON.parse(attempt.body).generationConfig.maxOutputTokens === 256));
});
...
test("responses adapter uses bounded output budget for visible-text primary probes", () => {
  ...
  assert.equal(JSON.parse(attempt.body).max_output_tokens, 256);
  assert.equal(
...
test("chat completions adapter uses enough output budget to emit visible text", () => {
  ...
  assert.equal(JSON.parse(attempt.body).max_tokens, 256);
...
test("gemini adapter uses the same primary probe instruction", () => {
  ...
  assert.equal(JSON.parse(attempt.body).generationConfig.maxOutputTokens, 256);
});
