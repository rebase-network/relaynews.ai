...
- 强化 Relay 详情页的 Base URL、健康摘要和模型列表标题。
...
- 问题：详情页顶部未展示被测试 Base URL；模型健康列表缺少整体摘要和标题。
- 方案：在 Relay hero meta 中展示 Base URL；联系信息识别 URL、邮箱、Telegram handle；模型健康 panel 增加“支持模型健康概览”、健康/异常/最近验证/样本覆盖摘要。
- 验收：`/relay/aurora-relay` 可见 `Base URL` 与 `aurora.relaynew.ai/v1`，模型健康区域有标题和摘要指标。
...
- P0：移动端榜单横向溢出。
- P1：Relay 详情摘要/Base URL、赞助语言、sitemap、favicon。
